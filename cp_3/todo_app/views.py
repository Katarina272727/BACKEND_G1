...
def todo_list_handler(request, pk):
    ...
    if result['status'] == 404:
        return JsonResponse({'message': 'TodoLists not found'}, status=404, safe=False)
    ...
    if request.method == 'DELETE':
        todo_list.delete()
        return JsonResponse({'message': 'TodoList is deleted successfully!'}, status=200, safe=False)
    return JsonResponse({'message': 'Request is not supported'}, status=400, safe=False)
...
def todo_list_todos_handler(request, pk):
    ...
    if result['status'] == 404:
        return JsonResponse({'message': 'TodoList not found'}, status=404, safe=False)
